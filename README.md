...
El operador resta (operator-) funciona únicamente para tensores con dimensiones exactamente iguales (incluyendo 1D, 2D y 3D). Realiza una validación de cada dimensión y luego recorre el arreglo lineal restando elemento a elemento.

El operador de multiplicación (operator*) funciona de dos maneras distintas. La primera es la multiplicación de un tensor con otro tensor, donde se recorre y se multiplica cada par de elementos, y el otro caso es la multiplicación de un tensor por un número, donde se recorre todo el arreglo data y se construye el vector con resultados nuevos después de haber sumado cada elemento con un valor constante. 
