# Desarrollo de la librería Tensor++
El programa se centra en la construcción de la librería **Tensor++**, la cual abre diferentes posibilidades en el manejo de tensores unidimensionales, bidimensionales y tridimensionales. Como muestra de la versatilidad de esta nueva librería, se construyó una **red neuronal** basada en el uso de tensores bidimensionales y tridimensionales. En esta aplicación se trato de implementar las diversas funciones y operaciones que permite realizar la librería en cuanto al menajeo de tensores.
...
A modo de complemento, el destructor (~Tensor) libera la memoria dinámica utilizando delete[] data. 
...
Adicionalmente, la librería 'Tensor++' cuenta con las funciones 'view' y 'unsqueeze' que le permiten al usuario modificar las fimensiones de Tensores que ya han sido creados.

La función 'view' permite cambiar la organización lógica de los datos, siempre y cuando las nuevas dimensiones ingresadas coincidan con las dimensiones actuales del tensor. Para explicar el cambio en la organización, es menester resaltar que los datos estan almacenados en un array de una dimensión, por lo cual no es necesario alterar el orden en el que estos se han guardado, únicamente modificamos las dimensiones. En cierto punto podríamos definir como **abstracta** la organización de los datos, ya que no existe como tal sino que se interpreta usando las dimensiones registradas.
En cuanto a la funcion 'unsqueeze', esta utiliza una mecánica similar a la función 'view', puesto que para integrar la nueva dimensión de tamaño 1, unicamente se modifica el vector que contiene las dimensiones del tensor. No obstante, tambien se realiza un proceso de verificación previa para evaluar si las dimensiones actuales del tensor permiten agregar una nueva dimensión. Finalmente, estas funciones retornan un tensor con las nuevas dimensiones pero con los mismo valores.

Adicionalmente, otra de las funciones más resaltantes de la librería es 'concat', la cual permite concatenar dos tensores en un eje seleccionado por el usuario. En esta función se realiza una verificación extensiva para garantizar que el eje ingresado es válido, que los tensores ingresados tienen datos para concatenar y estos tensores son de igual dimensionalidad. Para esta función, se requiere pasar el vector de tensores por copia, ya que se modificarán las dimensiones de estos objetos. Esta modificación consiste en que dependiendo de las dimensiones que ya tiene el tensor, se agrega nuevas dimensiones con valor 1 para siempre trabajar con tensores 3D. Ello se hace con la finalida de 'homogenizar' las operaciones y facilitar el proceso de concatenación. Posteriormente, se utilizan estructras selectivas para evaluar el eje seleccionado por el usuario, y a traves de bucles -for anidados se iteran los dos tensores para crear el nuevo tensor concatenado. Un aspecto a resaltar es que por cada eje se realiza una validación adicional a las dimensiones de los tensores, ya que para concatenar, las dimensiones de los dos tensores deben ser iguales a excepcion de aquella que corresponde al eje a traves del cual se concatenarán los tensores. Finalmente, se utiliza un constructor vacío para generar un nuevo tensor y registrar los nuevos datos.

## Sobrecarga adicional
De manera adicional a los requerimientos de la tarea, se desarrolló una sobrecarga del operador '<<' para imprimir los datos de los tensores en la consola. Como se mencionó anteriormente, la organización de valores del tensor es bastante abstracta, por lo cual solo es necesario impimir los datos y dependiendo del valor de las dimensiones realizar saltos de linea y/o cambios de capa.
Dado que los tensores pueden ser unidimensionales, bidimensinales o tridimensionales se utiliza estructras de control selectivas para realizar un metodo de impresión personalizado.

## Archivo *main*
Como se especificó anteriormente, para resaltar las diferentes funcionalidades de la librería, se construyó una red neuronal en el archivo *main.cpp*. Pora cada operación y creación de nuevos tensores, se utilizaron los getters de la clase para imprimir las dimensiones de estos tensores y evidenciar como varían. Finalmente, se uso la sobrecarga del operador '<<' para imprimir la versión final del tensor.
